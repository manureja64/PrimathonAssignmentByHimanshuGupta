...
exports.direction_create_get = function (req, res, next) {
    ...
    res.render('direction_excel_form', { title: 'Create Direction', id: req.params.id, direction: null, errors: null })
    ...
    (req, res, next) => {
        ...
        readXlsxFile(filePath, { schema }).then(({ rows, errors }) => {
            for (let i = 0; i < rows.length; i++) {
                const exDirection = new Direction(
                    {
                        direction: rows[i].direction,
                        test: req.params.id
                    }
                )
                exDirection.save(function (err) {
                    if (err) {
                        return next(err);
                    }
                    else {
...
exports.direction_update_post = [
    ...
    (req, res, next) => {
        ...
        const errors = validationResult(req);
        var ques = [];
        // The below async.parallel() and direction=new Direction {} ensures that the direction updated consists of question._id of all the questions contained in the original direction
        async.parallel({
            directionF: function (callback) {
                Direction.findById(req.params.id).populate('test').populate('question').exec(callback);

            },
        }, function (err, results) {
            if (err) { return next(err); }
            if (results.directionF == null) {
                var err = new Error('Direction not found');
                err.status = 404;
                return next(err);
            }
            //Success.
            // direction.question = results.directionF.question;
            var direction = new Direction(
                {
                    direction: req.body.direction,
                    test: req.body.test,
                    question: JSON.parse(JSON.stringify(ques)),
                    _id: req.params.id
                }
            );
            for (var i = 0; i < results.directionF.question.length; ++i) {
                direction.question.push(results.directionF.question[i]._id);
            }
            // console.log(results.directionF.question);
            // console.log(ques);
            // console.log(direction.question);
