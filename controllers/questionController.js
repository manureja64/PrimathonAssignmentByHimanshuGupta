...
exports.question_create_get = function (req, res, next) {
    ...
    async.parallel({
        ...
        async.parallel({
            ...
            //Successful, so render
            res.render('question_excel_form', { title: 'Create Question', numOfOptions: results1.direction_test.numOfOptions, id: req.params.id, question: null, errors: null });
            ...
            async.parallel({
                ...
                readXlsxFile(filePath, { schema }).then(({ rows, errors }) => {
                    for (let i = 0; i < rows.length; i++) {
                        ...
                        var exQuestion = new Question(
                            {
                                test: results6.test._id,
                        ...
                        exQuestion.save(function (err2) {
                            if (err2) { return next(err2); }

                        })
                        var que = exQuestion._id;
                        results.direction.question.push(que);
                        // results.direction.save();

                        results6.test.question.push(que);
                        results.direction.save();
                        results6.test.save();
                ...
                // results.direction.save();
                // results6.test.save();
                res.redirect(results.direction.url);
